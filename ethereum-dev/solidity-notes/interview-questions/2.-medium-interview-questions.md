...
Cold reads happen when a storage slot is read for the first time during a transaction call. 2.11 What is the difference between a cold read and a warm read?

Cold reads happen when a storage slot is read for the first time during a transaction call. Warm reads are for each subsequent call and have a lower gas cost than the initial cold read.
